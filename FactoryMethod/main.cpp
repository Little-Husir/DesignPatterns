...
依赖注入（Dependency Injection, DI）是一种设计模式，用于实现对象之间的依赖关系管理，使得对象的创建与依赖的管理更加灵活和可维护。
依赖注入的核心思想是将依赖关系通过外部注入到对象中,而不是在对象内部自行创建或管理这些依赖。

       简单一点来说就是在传递参数时，传入一个类，即该成员函数依赖这个类
